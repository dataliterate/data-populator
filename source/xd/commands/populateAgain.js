  ...
  try {
    ...
    if (activeConfiguration.type === 'preset') {
      try {
        ...
        log(e1)
        await Gui.createAlert(Strings(STRINGS.POPULATING_FAILED), Strings(STRINGS.UNABLE_TO_LOAD_LAST_USED_PRESET))
        return
      }
      ...
      try {
        let activeConfigurationJSONURL = await Data.loadFileInDataFolder('activeConfigurationJSONURL.json')
        data = await global.fetch(activeConfigurationJSONURL.url, {
          headers: activeConfigurationJSONURL.headers
        }).then(response => response.json())
        data = Utils.accessObjectByString(data, activeConfigurationJSONURL.key)
      } catch (e1) {
        log(e1)
        await Gui.createAlert(Strings(STRINGS.POPULATING_FAILED), Strings(STRINGS.UNABLE_TO_LOAD_JSON_AT_LAST_USED_URL))
        return
      }
    }
  } catch (e2) {
    log(e2)
    await Gui.createAlert(Strings(STRINGS.NO_ACTIVE_CONFIGURATION), Strings(STRINGS.FIRST_TIME_USING_DATA_POPULATOR))
    return
  }
