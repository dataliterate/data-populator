...
export default async (selection, root) => {
  ...
  await Gui.showPopulatorDialog('JSONURL', options, {lastUsedUrl, lastUsedJSONKey, lastUsedHeaders, showAdditionalOptions})
    .then(result => {
      url = result.url
      JSONKey = result.key
  ...
  if (!canceled) {
    ...
    else if (!Utils.isValidURL(url)) await Gui.createAlert(Strings(STRINGS.INVALID_URL), Strings(STRINGS.URL_ENTERED_INVALID))
  ...
  if (!data) {
    await Gui.createAlert(Strings(STRINGS.POPULATING_FAILED), Strings(STRINGS.UNABLE_TO_LOAD_JSON_AT_URL))
    return
  }
