...
const types = xd('uxp.storage.types')
const fsFormats = xd('uxp.storage.formats')
  ...
  return new Promise(async (resolve, reject) => {
    ...
    try {
      ...
      if (presets) {
        ...
        let presetsJSONFiles = presetsEntries.filter(entry => entry.name.indexOf('.json') >= 0 && entry.isFile)
...
export function downloadPresets () {
  return new Promise(async (resolve, reject) => {
    ...
    try {
      ...
      let presets = await global.fetch(baseUrl)
        .then((res) => {
          return res.text()
        })
        .then((data) => {
          ...
          return urlsToDownload.map(url => global.fetch(baseUrl + url).then((response) => response.json()))
        })
        .then(fetchList => {
      ...
      let presetsFolder = dataFolderEntries.filter(entry => entry.name === 'presets' && entry.isFolder)[0]
