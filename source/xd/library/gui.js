...
const fs = xd('uxp.storage.localFileSystem')
...
export async function showPopulatorDialog (type, options, data) {
  return new Promise(async (resolve, reject) => {
    ...
    if (type === 'preset') {
      ...
      ${getSubTitleSegment(Strings(STRINGS.PRESET), 'select-preset', false, 'margin-bottom-1')}
      <input type="text" class="hidden" autofocus="true"/>
      <label class="select-container">
        <select id="preset-select">
          ${data.presets.map((preset, i) => '(<option value="' + i + '">' + data.paths[preset.nativePath].split('presets/')[1] + '</option>)')}
        </select>
      </label>
      `
    } else if (type === 'JSON') {
      ...
      config.JSONSegment = `
      ${getSubTitleSegment(Strings(STRINGS.JSON_FILE), 'json-file', false, 'margin-bottom-6')}
      <input type="text" class="hidden" autofocus="true"/>
      <label class="input-container">
        <input readonly="true" id="json-file-input" type="text" /><button id="json-file-browse-button">Browse</button>
      </label>
      `
      ...
      <div class="url-group">
        <div class="url-title-container">
          <span class="title json-url">${Strings(STRINGS.URL)}</span>
          <div id="show-additional-options-button"></div>
        </div>
        <label class="input-container">
          <input id="json-url-input" placeholder="${Strings(STRINGS.URL_PLACEHOLDER)}" type="text" />
        </label>

        <div class="additional-options">
          <span class="title headers">${Strings(STRINGS.HEADERS)}</span>
          <label class="headers-container"></label>
          <div class="add-header-button-container">
      ...
      <form method="dialog">
        ...
        <div class="left-right-container">
          ...
          <div class="left">
            ...
            ${getSubTitleSegment(Strings(STRINGS.DATA_PATH), 'json-key', Strings(STRINGS.DATA_PATH_HELP_TEXT), 'margin-top-19 margin-bottom-6')}
            <label class="input-container">
              <input id="json-key-input" placeholder="${Strings(STRINGS.DATA_PATH_PLACEHOLDER)}" type="text"/>
            </label>

            ${getSubTitleSegment(Strings(STRINGS.DATA_OPTIONS), 'data-options', false, 'margin-top-19 margin-bottom-3')}
            <label class="row">
            ...
            ${getSubTitleSegment(Strings(STRINGS.DEFAULT_SUBSTITUTE), 'default-substitute', Strings(STRINGS.DEFAULT_SUBSTITUTE_HELP_TEXT), 'margin-top-15 margin-bottom-6')}
            <label class="input-container">
              <input id="default-substitute-input" placeholder="${Strings(STRINGS.DEFAULT_SUBSTITUTE_PLACEHOLDER)}" type="text"/>
            </label>
          </div>
    ...
    document.getElementsByTagName('form')[0].addEventListener('keydown', (e) => {
      if (e.keyCode === 27) {
        document.getElementsByTagName('form')[0].removeChild(document.getElementsByClassName('left-right-container')[0])
    ...
    JSONPreviewReloadButton.addEventListener('click', async () => {
      if (type === 'preset') {
        let path = data.paths[data.presets[presetSelect.value].nativePath]
        updateJSONPreviewPreset(path, false)
      } else if (type === 'JSON') {
    ...
    async function updateJSONPreviewJSONFile (file, newSelection) {
      ...
      if (file !== false) {
        ...
        getJSONPreview(jsonPreviewData)
        ...
        if (JSONFile.hasOwnProperty('nativePath') && JSONFile.hasOwnProperty('json')) {
          ...
          JSONFile.nativePath = nativePath
          JSONFile.json = jsonData
        } else if (data.lastUsedJSONFile.hasOwnProperty('nativePath') && data.lastUsedJSONFile.hasOwnProperty('json')) {
    ...
    async function updateJSONPreviewJSONURL (url, load) {
      ...
      if (url && Utils.isValidURL(url)) {
        ...
        try {
          ...
          jsonData = await global.fetch(url, {
            headers
        ...
        else if (!Utils.isValidURL(url)) showDataPreviewError(JSONPreviewDIV, Strings(STRINGS.INVALID_URL))
      }
    }
    ...
    function updateHeaders () {
      ...
      for (let i = 0; i < headerElems.length; i++) {
        ...
        if (headerNameElem.value && headerValueElem.value) headers[headerNameElem.value] = headerValueElem.value
      }
    }
...
function questionMarkEnterHandler (e) {
  ...
  tooltipElem.style.left = 34 + questionMark.offsetLeft + 16 + 6 + 5

  if (questionMark.parentNode.getElementsByClassName('h1-title').length > 0) {
    tooltipElem.style.top = questionMark.parentNode.offsetTop + 3 - 7
  } else {
    tooltipElem.style.top = questionMark.parentNode.offsetTop - 7
  }
...
function updateHeaderContainerHeight (add) {
  ...
  if (length > 2) {
    headersContainer.style.overflowY = 'scroll'
    headersContainer.style.height = (1 + 1) * 28 - 4
    headersContainer.scrollTop = headersContainer.scrollHeight
  } else {
    headersContainer.style.overflowY = 'auto'
    headersContainer.style.height = length * 28 - 4
  }

  if (length === 0) {
    headersContainer.style.marginBottom = 0
  } else {
    headersContainer.style.marginBottom = 7
  }
}
...
function getPopulatorUIStyles () {
  ...
  .sub-title-container.margin-top-19 {
    margin-top: 19px;
  }
  .sub-title-container.margin-top-15 {
    margin-top: 15px;
  }
  .sub-title-container.margin-bottom-7 {
    margin-bottom: 7px;
  }
  .sub-title-container.margin-bottom-6 {
    margin-bottom: 6px;
  }
  .sub-title-container.margin-bottom-3 {
    margin-bottom: 3px;
  }
  .sub-title-container.margin-bottom-2 {
    margin-bottom: 2px;
  }
  .sub-title-container.margin-bottom-1 {
    margin-bottom: 1px;
  }
  ...
  .title-container div.question-mark,
  ...
  .title-container div.question-mark span,
  .sub-title-container div.question-mark span {
    position: absolute;
    left: 1px;
    top: -1px;
  ...
  .title-container div.question-mark:hover,
  .sub-title-container div.question-mark:hover {
    border: 2px solid #BBB;
  }
  .title-container div.question-mark:hover span,
  .sub-title-container div.question-mark:hover span {
    color: #BBB;
  }
  ...
  .url-group .url-title-container span.title.json-url {
    margin-bottom: 6px;
    margin-left: 4px;
    margin-right: 6px;
  }
  .url-group .url-title-container #show-additional-options-button {
    position: relative;
  ...
  .url-group .url-title-container #show-additional-options-button:hover {
    background-size: cover;
    background-position: center;
    background-repeat: no-repeat;
  ...
  .url-group .input-container {
    margin-left: 6px;
  }

  .url-group span.title.headers {
    margin-top: 16px;
    margin-bottom: 6px;
    margin-left: 6px;
  }
  .url-group .additional-options {
...
function getSubTitleSegment (title, className, description, containerClassName) {
  return `
  <div class="sub-title-container ${containerClassName}">
    <span class="title ${className}">${title}</span>
    ${(description) ? `
...
function getJSONStyles () {
  ...
  .json-preview-container #json-preview-reload-button {
    position: absolute;
    top: 18px;
    right: 20px;
  ...
  .json-preview-container .footer .button-container button {
    padding: 0;
    margin: 0;
  }
...
function getJSONPreviewSegment (refreshButton, populateButton, cancelButton, okButton) {
  ...
  <div class="json-preview-container">
    ...
    ${refreshButton ? `
      <button id="json-preview-reload-button" uxp-variant="cta">${Strings(STRINGS.RELOAD)}</button>
    ` : ''}
...
function getJSONPreview (_data) {
  ...
  if (!_data) {
    ...
    lines = lines.map((line, index) => {
      ...
      return `
      <div id="line-${index}" class="line${hiddenLine ? ` hidden hidden-${hiddenStartLineIndex}` : ''}">
        ...
        ${(valueClass === 'url' || valueClass === 'string') ? `
          ...
          <span ${(valueClass === 'url') ? `title="${lineElements.fullURL}"` : ''} class="value ${valueClass}">${lineElements.value}</span>
    ...
    if (linesSlicedTo250) {
      let endLine = '<div class="line"><span class="line-number digits-' + lineNumberDigits + '">.</span><div class="fold"></div></div>'
      lines.push(endLine)
      lines.push(endLine)
      lines.push(endLine)
    }
    ...
    linesContainerDIV.addEventListener('click', async (e) => {
      if ((['arrow', 'arrow folded'].indexOf(e.target.className) > -1 && e.target.tagName === 'DIV') ||
        ...
        if (e.target.className.indexOf('arrow') > -1 && e.target.tagName === 'DIV') {
          ...
          arrow = e.target.parentNode.getElementsByClassName('fold')[0].getElementsByClassName('arrow')[0]
        }
        ...
        if (arrow.className === 'arrow') {
          ...
          for (let i = arrowIndex + 1; i <= foldEndIndex; i++) {
            if (lineDIVs[i].className === 'line') lineDIVs[i].className = 'line hidden hidden-' + arrowIndex
          }
 ...
 * @param {Object} json
 */
export async function showActiveConfigurationDialog (activeConfiguration, activeConfigurationSpecific, options, data) {
  return new Promise(async (resolve, reject) => {
    let config = {}
    if (activeConfiguration.type === 'preset') {
      config.type = Strings(STRINGS.POPULATE_WITH_PRESET_TITLE)
      config.presetSegment = `
      ${getSubTitleSegment(Strings(STRINGS.PRESET), 'preset-path', false, 'margin-bottom-7')}
      <label class="input-container">
      ...
      if (Object.keys(activeConfigurationSpecific.headers).length > 1) {
        ...
        config.JSONURLSegment = `
        <div class="url-title-container">
          ${getSubTitleSegment(Strings(STRINGS.URL), 'json-url', false, 'margin-bottom-6')}
        </div>
        <label class="input-container">
          <input readonly="true" id="json-url-input" type="text" />
        </label>
    ...
    <dialog id="dialog">
      <form method="dialog">
        ...
        <div class="left-right-container">
          ...
          <div class="left">
            ${getTitleSegment(Strings(STRINGS.LAST_USED_DATA_TITLE), Strings(STRINGS.LAST_USED_DATA_DESCRIPTION))}

            ${getSubTitleSegment(Strings(STRINGS.COMMAND), 'populate-command', false, 'margin-bottom-2')}
    ...
    // set initial values
    if (activeConfiguration.type === 'preset') pathInput.value = activeConfigurationSpecific.path.split('presets/')[1]
    if (activeConfiguration.type === 'JSON') jsonInput.value = activeConfigurationSpecific.nativePath.split('/')[activeConfigurationSpecific.nativePath.split('/').length - 1]
