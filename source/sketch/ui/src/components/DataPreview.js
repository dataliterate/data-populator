...
class DataPreview extends React.Component {
  ...
  checkWhetherToShowOverlay () {
    if (this.props.loading || this.props.apiURLInvalid || this.props.invalidData || !this.props.data) {
    ...
    if (this.props.loading) {
      ...
      this.setState({
        showOverlay: true,
        loading: false,
        overlayMessage: (this.props.type !== 'url') ? Strings(STRINGS.UNABLE_TO_PREVIEW_JSON) : Strings(STRINGS.UNABLE_TO_LOAD_JSON_AT_URL)
      })
      ...
      this.setState({
        showOverlay: true,
        loading: false,
        overlayMessage: (this.props.type !== 'url') ? Strings(STRINGS.NO_FILE_SELECTED) : Strings(STRINGS.NO_URL_ENTERED)
      })
    } else {
      this.setState({
        showOverlay: false
  ...
  getHTML () {
    ...
    lines = lines.map((line, index) => {
      ...
      return (
        <div id={`line-${index}`} key={`line-${index}`} className={`line${hiddenLine ? ` hidden hidden-${hiddenStartLineIndex}` : ''}`}>
          <p className='line-number' style={{ width: 12 + 8 * this.lineNumberDigits }}>{index + 1}</p>
          ...
          {(valueClass === 'url' || valueClass === 'string') ? (<p className='value-quote-start'>{'"'}</p>) : ''}
          {(valueClass === 'url' || valueClass === 'string') ? (<p title={(valueClass === 'url') ? lineElements.fullURL : undefined} className={`value ${valueClass}`}>{lineElements.value}</p>) : ''}
          {(valueClass === 'url' || valueClass === 'string') ? (<p className='value-quote-end'>{'"'}</p>) : ''}

          {(valueClass !== 'url' && valueClass !== 'string') ? (<p className={`value ${valueClass}`}>{lineElements.value}</p>) : ''}
          {hasComma ? (
            <p className='comma'>{','}</p>
          ) : ''}
          {hiddenStartLine ? (
          ...
          {(valueClass === 'url') ? (
            <p className='full-url'>{lineElements.fullURL}</p>
          ) : ''}
  ...
  handleClick (e) {
    ...
    if (['arrow', 'arrow folded'].indexOf(e.target.className) > -1 || e.target.className === 'dots') {
      let data = JSON.parse(JSON.stringify(this.props.data))
      let string = JSON.stringify(data, null, '\t')
      ...
      <div ref='dataPreview' className={dataPreviewClass} onClick={this.handleClick}>
        ...
        <div className='overlay-container'>
          <div className='overlay-content'>
            {this.state.loading ? (<div className='spinner' />) : ''}<p>{this.state.overlayMessage}</p>
          </div>
        </div>
