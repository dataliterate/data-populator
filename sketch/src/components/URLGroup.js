    ...
    return (
      <div className={urlGroupClass}>
        ...
        <TextField
          ...
          autoFocus
          name={OPTIONS.URL}
          placeholder={Strings(STRINGS.URL_PLACEHOLDER)}
          value={this.props.urlValue}
          handleChange={this.props.handleURLChange}
        />

        {(!this.props.readOnly && this.state.headersVisible) ||
        (this.props.readOnly && (this.props.headers || []).length) ? (
          <div>
            <Title subTitle title={Strings(STRINGS.HEADERS)} />
            <HeadersList
              readOnly={this.props.readOnly}
              headers={this.props.headers}
              handleHeadersChange={this.props.handleHeadersChange}
              handleRemoveHeaderButton={this.props.handleRemoveHeaderButton}
            />
            {!this.props.readOnly ? (
              <Button
                style={{ marginBottom: 18 }}
                small
                text={Strings(STRINGS.ADD)}
                handleClick={this.props.handleAddHeaderButton}
              />
            ) : (
              ''
            )}
        ...
        {!this.props.readOnly ? (
          <Button
            disabled={this.props.urlValue.length === 0}
            style={{ width: '100%' }}
            blue
            text={Strings(STRINGS.LOAD)}
            handleClick={this.props.handleLoadURLButtonClick}
          />
        ) : (
          ''
        )}
