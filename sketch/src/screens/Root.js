...
class Root extends React.Component {
  ...
  init(params) {
    ...
    if (
      params.presets &&
      !params.presets.filter(preset => preset.path === (params.options.selectedPreset || {}).path)
        .length
    ) {
    ...
    this.setState(
      {
        ...
        options: Object.assign({}, params.options, {
          [OPTIONS.POPULATE_TYPE]: Utils.getPropertyValue(
            params.options,
            OPTIONS.POPULATE_TYPE,
            null
          ),
          [OPTIONS.RANDOMIZE_DATA]: Utils.getPropertyValue(
            params.options,
            OPTIONS.RANDOMIZE_DATA,
            true
          ),
          [OPTIONS.TRIM_TEXT]: Utils.getPropertyValue(params.options, OPTIONS.TRIM_TEXT, true),
          [OPTIONS.INSERT_ELLIPSIS]: Utils.getPropertyValue(
            params.options,
            OPTIONS.INSERT_ELLIPSIS,
            true
          ),
          [OPTIONS.DEFAULT_SUBSTITUTE]: Utils.getPropertyValue(
            params.options,
            OPTIONS.DEFAULT_SUBSTITUTE,
            ''
          ),
          ...
          [OPTIONS.COLUMNS_MARGIN]: Utils.getPropertyValue(
            params.options,
            OPTIONS.COLUMNS_MARGIN,
            10
          ),
          ...
          [OPTIONS.SELECTED_PRESET]: Utils.getPropertyValue(
            params.options,
            OPTIONS.SELECTED_PRESET,
            null
          )
        })
      },
      () => {
        ...
        $(document).trigger('setTextFieldValue', {
          ...
          value: this.state.options[OPTIONS.DATA_PATH]
        })
        $(document).trigger('setTextFieldValue', {
  ...
  reloadData(keepDataPath, triggeredByClick) {
    if (this.state.options[OPTIONS.POPULATE_TYPE] === OPTIONS.POPULATE_TYPE_PRESET) {
      ...
      // get data from url
      global
        .fetch(this.state.options[OPTIONS.URL], {
          headers
        })
            ...
            <div className="content">
              {this.state.options[OPTIONS.POPULATE_TYPE] === OPTIONS.POPULATE_TYPE_PRESET ? (
                <div>
                  ...
                  {!this.state.viewOnly ? (
                    <Select
                      data={this.state.presets.map(preset => preset.name)}
                      selected={(this.state.options[OPTIONS.SELECTED_PRESET] || {}).name}
                      handleChange={this.handlePresetsSelectChange}
                    />
                  ) : (
                    <TextField
                      readOnly
                      value={(this.state.options[OPTIONS.SELECTED_PRESET] || {}).name}
                    />
                  )}
                </div>
              ) : (
              ...
              {this.state.options[OPTIONS.POPULATE_TYPE] === OPTIONS.POPULATE_TYPE_JSON ? (
                <div>
                  ...
                  <FileBrowseField
                    ...
                    file={this.state.jsonFile}
                    onClick={this.handleJSONBrowse}
                  />
                </div>
              ) : (
                ''
              ...
              <Title
                subTitle
                title={Strings(STRINGS.DATA_PATH)}
                description={Strings(STRINGS.DATA_PATH_HELP_TEXT)}
              />
              <TextField
                readOnly={this.state.viewOnly}
                name={OPTIONS.DATA_PATH}
                placeholder={Strings(STRINGS.DATA_PATH_PLACEHOLDER)}
                value={this.state.options[OPTIONS.DATA_PATH] || ''}
              ...
              <Checkbox
                readOnly={this.state.viewOnly}
                name={OPTIONS.RANDOMIZE_DATA}
                label={Strings(STRINGS.RANDOMIZE_DATA_ORDER)}
                checked={this.state.options[OPTIONS.RANDOMIZE_DATA]}
                handleChange={this.handleCheckboxChange}
              />
              <Checkbox
                readOnly={this.state.viewOnly}
                name={OPTIONS.TRIM_TEXT}
                label={Strings(STRINGS.TRIM_TEXT)}
                checked={this.state.options[OPTIONS.TRIM_TEXT]}
                handleChange={this.handleCheckboxChange}
              />
              <Checkbox
                readOnly={this.state.viewOnly}
                marginBottom
                name={OPTIONS.INSERT_ELLIPSIS}
                label={Strings(STRINGS.INSERT_ELLIPSIS)}
                checked={this.state.options[OPTIONS.INSERT_ELLIPSIS]}
                handleChange={this.handleCheckboxChange}
              />

              <Title
                subTitle
                title={Strings(STRINGS.DEFAULT_SUBSTITUTE)}
                description={Strings(STRINGS.DEFAULT_SUBSTITUTE_HELP_TEXT)}
              />
              <TextField
                readOnly={this.state.viewOnly}
                name={OPTIONS.DEFAULT_SUBSTITUTE}
                placeholder={Strings(STRINGS.DEFAULT_SUBSTITUTE_PLACEHOLDER)}
                value={this.state.options[OPTIONS.DEFAULT_SUBSTITUTE]}
                handleChange={this.handleTextFieldChange}
              />
              ...
              <Checkbox
                ...
                readOnly={this.state.viewOnly}
                name={OPTIONS.CREATE_GRID}
                label={Strings(STRINGS.CREATE_GRID)}
                checked={this.state.options[OPTIONS.CREATE_GRID]}
                handleChange={this.handleCheckboxChange}
              />
                ...
                {!this.state.viewOnly ? (
                  <div>
                    <Button
                      ...
                      handleClick={this.handleCancelButtonClick}
                    />
                    <Button
                      disabled={
                        this.state.loading || this.state.jsonDataInvalid || !this.state.jsonData
                      }
                      blue
                      text={Strings(STRINGS.POPULATE)}
                      handleClick={this.handlePopulateButtonClick}
                    />
                  </div>
                ) : (
                  <Button
                    blue
                    text={Strings(STRINGS.OK)}
                    handleClick={this.handleCancelButtonClick}
                  />
                )}
              </div>
