...
export function parseAction(actionString) {
  ...
  let conditionPlaceholders = Core.placeholders.extractPlaceholders(conditionString)

  // get params
  actionComponents.shift()
  let params = actionComponents.map(paramString => {
    // get placeholders in param
    let paramPlaceholders = Core.placeholders.extractPlaceholders(paramString)
...
export function resolveAction(action, data) {
  ...
  action.condition.placeholders.forEach(placeholder => {
    ...
    let populatedPlaceholder = Core.placeholders.populatePlaceholder(placeholder, data, 'null')
  ...
  try {
    ...
    Context().document.showMessage(
      "Conditional action '" + action.string + "' could not be evaluated."
    )
  }
...
export function performAction(action, layer, data) {
  ...
  if (!actionFunction) {
    return Context().document.showMessage(
      "Conditional action '" + action.command + "' on layer '" + layer.name() + "' does not exist."
    )
  }
  ...
  try {
    ...
    // show error that action could not be evaluated
    Context().document.showMessage(
      "Conditional action on layer '" + layerName + "' could not be evaluated."
    )
  }
  ...
  try {
    ...
    Context().document.showMessage(
      "Conditional action on layer '" + layerName + "' could not be performed."
    )
  }
}
