...
export function loadPresets() {
  ...
  let enumerator = NSFileManager.defaultManager().enumeratorAtURL_includingPropertiesForKeys_options_errorHandler(
    url,
    [NSURLIsDirectoryKey, NSURLNameKey, NSURLPathKey],
  ...
  while (fileUrl) {
    ...
    if (fileUrl.pathExtension().isEqualToString('json')) {
      ...
      if (!Number(isDir.value())) {
        ...
        let presetDisplayPath = presetPath.stringByReplacingOccurrencesOfString_withString(
          presetsPath + '/',
          ''
        )
